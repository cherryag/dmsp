...
% version July 2018
% UPDATED: 12/12/2019 (normalize by t2 average YFP)
...
% load relevant data
analysis_kinetics = uigetdir; % where all the .mat files are located
...
%% 1) collect all fluorescence intensities and do a scatter plot [optional]
...
%% 2) visualize RFP vs. YFP and TFP vs. YFP of each cell before normalization [optional]
% OPTIONAL.
...
%% 3) divide RFP and TFP value of each cell by the average t2 YFP value for that expt condition

% average YFP value at t2
t = 2; 
for q = 1:total_cond
    t2_y_norm(q) = nanmean(yfp_fintcells_q_ypos{t,q});
end
...
for q = 1:total_cond
    ...
    %     end
        ...
        for i = 1:length(rfp_cells_temp); % refer to each cell in the image   

            % normalize each cell by its YFP value [UPDATED 12/12/2019]
            if isnan(yfp_cells_temp(i)) == 0; % if the cell fluo value is not NaN
               rfp_ynorm_temp(i) = rfp_cells_temp(i)./  t2_y_norm(q); % yfp_cells_temp(i);
               tfp_ynorm_temp(i) = tfp_cells_temp(i)./  t2_y_norm(q); % yfp_cells_temp(i);
            else
...
%% 7) collect all fluorescence intensities AFTER YFP normalized [optional]
...
%% 8) visualize RFP vs. YFP and TFP vs. YFP of each cell AFTER YFP normalization  [optional]
% OPTIONAL.
...
% run at the end
save(['13_YFP_normalized_t2Y_Ythresh_',num2str(thresh_YFP),'.mat'],savevars{:});
